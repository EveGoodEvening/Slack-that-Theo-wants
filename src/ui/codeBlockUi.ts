...
/**
 * Progressive-enhancement script for the copy-code affordance. Reads
 * `code.textContent` (browser-unescaped) and writes it to the clipboard via
 * `navigator.clipboard` when available, with a `document.execCommand('copy')`
 * fallback for older browsers. Degrades silently when neither is available.
 */
...
/**
 * Progressive-enhancement script for the composer preview toggle. Wires any
 * element with `data-preview-for="<textareaId>"` to POST the textarea's raw
...
export const PREVIEW_SCRIPT = `
    (function () {
      document.querySelectorAll('[data-preview-for]').forEach(function (toggle) {
